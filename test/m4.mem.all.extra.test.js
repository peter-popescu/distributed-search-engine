    ...
    try {
      ...
      distribution.local.comm.send(
          ...
          (e, v) => {
            ...
            distribution.local.comm.send(
                messages[1],
                {node: expectedNodeForKey(keys[1]), service: 'mem', method: 'get'},
                (e, v) => {
                  try {
                    expect(e).toBeFalsy();
                  ...
                  distribution.local.comm.send(
                      ...
                      (e, v) => {
                        ...
                        distribution.local.comm.send(
                            messages[3],
                            {node: expectedNodeForKey(keys[3]), service: 'mem', method: 'get'},
                            (e, v) => {
                              try {
                                expect(e).toBeFalsy();
                              ...
                              distribution.local.comm.send(
                                  messages[4],
                                  {node: expectedNodeForKey(keys[4]), service: 'mem', method: 'get'},
                                  (e, v) => {
                                    try {
                                      expect(e).toBeFalsy();
